# Lyra 视觉生成协议 V6.3 — 深化融合小小东提示词工程

> 当 `[视觉]` 或 `[卡片]` 标签触发时加载本协议。  
> V6.3 升级：继续吸收小小东 2026-05-11 新增线稿海报方法，把“先检索后作图、参考图转译、知识型页面表达、PPT 页面化输出”纳入视觉编译协议。
...
7. **先检索后作图**（V6.3 新增）：知识型海报、信息页、教育页、工业/医学/历史类视觉任务，先理解事实和结构，再做视觉转译。
8. **页面化输出意识**（V6.3 新增）：不仅生成单图，也要支持封面、知识页、PPT 章节页、信息页等可承载内容的表达页面。
...
### 模式五：线稿海报 / PPT 页面（V6.3 新增）

**原版核心**：先检索后作图；不是把知识堆上页面，而是把可靠内容转译成高级线稿海报和可讲述页面。

**Lyra V6.3 槽位化**：
```xml
<research_first>
  检索优先：历史 / 医学 / 工业 / 教育 / 品牌 / 产品类信息，先调查再表达
  信息约束：核心事实、阶段、功能、关系、对比必须可信
</research_first>

<page_translation>
  页面类型：[海报 / 封面 / 知识页 / PPT章节页 / 信息页]
  文字承载：[标题 / 副标题 / 标注 / 要点 / 注释]
  页面目标：[3秒读懂 / 可讲述 / 可传播 / 可复用]
</page_translation>

<reference_rewrite>
  参考图用途：[主体结构 / 内容蓝本 / 版式线索 / 视觉风格]
  转译要求：统一成高级线稿海报语言，不保留杂乱原样拼贴感
</reference_rewrite>
